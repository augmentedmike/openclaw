...
import { Tn as sensitive } from "./model-selection-9r_l_jO9.js";
...
const FIELD_HELP = {
	...
	"logging.maxFileBytes": "Maximum size in bytes for a single log file before rotation triggers (default 500 MB). When the threshold is exceeded, the current file is renamed to file.1 and a new file is opened. Set maxBackups to control how many rotated files are retained.",
	"logging.maxBackups": "Number of rotated log backup files to keep alongside the active log file (default 5). When a rotation occurs, older backups are shifted up (file.1 → file.2, …) and any backup beyond this count is deleted. Set to 0 to disable rotation and revert to the legacy suppress-on-cap behaviour.",
