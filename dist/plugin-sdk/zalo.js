...
function resolveSettings() {
	...
	return {
		...
		maxBackups: resolveMaxBackups(cfg?.maxBackups)
	};
}
function settingsChanged(a, b) {
	if (!a) return true;
	return a.level !== b.level || a.file !== b.file || a.maxFileBytes !== b.maxFileBytes || a.maxBackups !== b.maxBackups;
}
...
function buildLogger(settings) {
	...
	logger.attachTransport((logObj) => {
		try {
			...
			if (nextBytes > settings.maxFileBytes) if (settings.maxBackups > 0) {
				...
				if (appendLogLine(settings.file, noticePayload)) currentFileBytes += Buffer.byteLength(noticePayload, "utf8");
				if (appendLogLine(settings.file, payload)) currentFileBytes += payloadBytes;
				return;
			} else {
...
function resolveMaxBackups(raw) {
	if (typeof raw === "number" && Number.isFinite(raw) && raw >= 0) return Math.floor(raw);
...
*/
function rotateLogFile(file, maxBackups) {
	try {
		const oldest = `${file}.${maxBackups}`;
		try {
			fs$1.rmSync(oldest, { force: true });
		} catch {}
		for (let i = maxBackups - 1; i >= 1; i--) {
			const src = `${file}.${i}`;
			const dst = `${file}.${i + 1}`;
			try {
				fs$1.renameSync(src, dst);
			} catch {}
		}
		try {
			fs$1.renameSync(file, `${file}.1`);
		} catch {}
	} catch {}
}
...
function pruneOldRollingLogs(dir) {
	try {
		...
		for (const entry of entries) {
			...
			const isActiveLog = entry.name.startsWith(`${LOG_PREFIX}-`) && entry.name.endsWith(LOG_SUFFIX);
			const isRotatedBackup = entry.name.startsWith(`${LOG_PREFIX}-`) && /\.log\.\d+$/.test(entry.name);
			if (!isActiveLog && !isRotatedBackup) continue;
