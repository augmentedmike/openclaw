#!/usr/bin/env node
import { Q as isRootVersionInvocation, Y as hasHelpOrVersion, Z as isRootHelpInvocation, t as createSubsystemLogger } from "./subsystem-BruFJSKI.js";
...
if (!isMainModule({
	...
	function tryHandleRootHelpFastPath(argv) {
		...
		import("./program-d67iSDWR.js").then(({ buildProgram }) => {
	...
	if (!ensureExperimentalWarningSuppressed()) {
		...
		if (!tryHandleRootVersionFastPath(process$1.argv) && !tryHandleRootHelpFastPath(process$1.argv)) import("./run-main-C3CXCF9w.js").then(({ runCli }) => runCli(process$1.argv)).catch((error) => {
